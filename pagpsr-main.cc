 ...
 std::cout << "Creating  " << (unsigned)size << " nodes .."<<" with "<<nPairs<<" pairs..." <<"\n";
  ...
  MobilityHelper mobility;
    mobility.SetPositionAllocator(
        ...
        StringValue("ns3::UniformRandomVariable[Min=0|Max=2000]"), "Y",
        StringValue("ns3::UniformRandomVariable[Min=0|Max=2000]"), "Z",
    ...
    for (size_t i = 0; i < size / 2; i++) {
        ...
        double x = uv->GetInteger(0, 2000);
        double y = uv->GetInteger(0, 2000);
        double xSpeed = uv->GetValue(15, 30);
        double ySpeed = uv->GetValue(15, 30);
    ...
    mobility.SetMobilityModel(
        ...
        "Distance", StringValue("200"), "Speed",
        StringValue("ns3::ConstantRandomVariable[Constant=" +
                    std::to_string(mspeed) + "]"),
        "Bounds", StringValue("0|2000|0|2000"));
    for (uint32_t i = size / 2; i < nodes.GetN(); i++) {
        mobility.Install(nodes.Get(i));
    }
  // Name nodes
